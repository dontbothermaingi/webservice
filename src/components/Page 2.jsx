...
function Page2 (){
    ...
    return ( 
      ...
      <div className="w-full max-w-7xl mx-auto px-4 py-8 flex flex-col text-center ">
        ...
        {/* Service Cards */}
        <div className="w-full max-w-7xl mx-auto mb-16 px-4 flex gap-9">
            {serviceProviders?.slice(0,3).map((service, index) => (
            <Motion index={index} key={index}>
                <Card 
                    className="bg-slate-800/50 border-slate-700/50 backdrop-blur-sm hover:bg-slate-800/70 transition-all duration-300 transform hover:shadow-xl hover:shadow-blue-500/10 w-100 h-110"
                >
                    ...
                    {service.more_details && service.more_details.map((detail,i) => (
                        <div key={i} className="flex justify-between items-start mb-3">
                            ...
                            <span className="bg-slate-700/50 text-slate-100 text-xs px-3 py-1 rounded-full font-medium">
                                {detail.category}
                            </span>
                    ...
                    {/* Service Image */}
                    <div className="w-full h-42 bg-gradient-to-br from-slate-700 to-slate-600 rounded-lg mb-3 flex items-center justify-center overflow-hidden">
                        <img 
                    ...
                    {service.more_details && service.more_details.map((detail,i) => (
                        <div key={i}>
                            <h3 className="text-black font-bold text-lg mb-2">{detail.jobTitle}</h3>
                        </div>
                    ))}

                    <div>
                            <p className="text text-sm mb-3 leading-relaxed">{services[0]?.description}</p>
                    </div>
